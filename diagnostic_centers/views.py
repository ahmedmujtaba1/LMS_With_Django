    ...
    for _, row in df.iterrows():
        ...
        if duplicate_check == 0:  
            ...
            insert_query = '''
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            '''
            conn.execute(insert_query, (supervisor, student1, student2, student3, predictions, ''))
            num += 3
...
def admin_dashboard(request, username=None):
    ...
    import pandas as pd
    from chat_messages.models import Room
    record_groups = fetch_records()
    df = pd.DataFrame(record_groups)
    num = 0
    supervisor_username = admin2.username
    student = list()
    num2 = 0
    for _, row in df.iterrows():
        num2 += 1
        supervisor = row['Supervisor']
        title = list(row["Predictions"])
        project_id = ''
        student1 = str(row['Students'][num])
        new_projects = list(NewProjectGroup.objects.values_list('pk', 'student1').distinct())
        for new_project in new_projects:
            new_project2 = new_project[1]
            if str(new_project2).lower() == str(student1).lower():
        ...
        if str(supervisor) == str(supervisor_username):
            student.append(
                {
                    'sid' : project_id,
    ...
    title = str(df["Predictions"]).split(',')[0]
...
#         conn.execute(insert_query, (supervisor, student1, student2, student3, predictions, False))
#         insert_query = '''
#             INSERT INTO chat_message_room (supervisor, student1, student2, student3, name)
#             VALUES (?, ?, ?, ?, ?, ?)
#         '''
#         conn.execute(insert_query, (supervisor, student1, student2, student3, predictions))
#         num += 3
